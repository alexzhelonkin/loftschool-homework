...
/*
 ...
 Пример:
   ...
   элементами div и span т.к. следующим соседом этих элементов является элемент с тегом P
 */
function findAllPSiblings(where) {
    return [...where.children]
        .filter(child => child.nextElementSibling && child.nextElementSibling.nodeName === 'P');
}
...
/*
 ...
 "элемент" внутри узла переданного в параметре where и возвращает массив из текстового содержимого найденных элементов
...
function deleteTextNodes(where) {
    [...where.childNodes].forEach(item => item.nodeType === 3 && item.remove());
}
...
function collectDOMStat(root) {
    ...
    function setStat(node) {
        const increment = (obj, s) => obj[s] = (obj[s] || 0) + 1;

        if (node.nodeType === 3) {
            return result.texts += 1;
        }

        [...node.classList].forEach((cls) => increment(result.classes, cls));

        increment(result.tags, node.nodeName);

        [...node.childNodes].forEach(setStat)
    }

    [...root.childNodes].forEach(setStat)
